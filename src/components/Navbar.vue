<template>
	<div class="sticky top-0 z-20">
		<header class="bg-musicca-crimson-560">
			<div class="max-w-7xl mx-auto px-2 sm:px-4 md:flex md:justify-between lg:px-8">
				<nav class="hidden md:flex md:py-2 md:space-x-4 lg:space-x-8" aria-label="Global navigation">
					<router-link
						to="/"
						class="text-gray-100 hover:bg-musicca-crimson-630 hover:text-white rounded-md py-2 px-3 inline-flex items-center text-sm font-semibold focus:outline-none focus-visible:ring-1 focus-visible:ring-white"
						active-class="bg-musicca-crimson-600"
					>
						Musicca
					</router-link>
				...
				<div class="relative h-16 flex md:max-w-md md:w-full lg:max-w-lg">
					<div class="relative z-10 flex items-center md:hidden">
						<button
							class="rounded-md p-2 inline-flex items-center justify-center hover:bg-musicca-crimson-630 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white focus:bg-musicca-crimson-630"
							:aria-label="`Switch to ${isDarkMode ? 'light theme' : 'dark theme'}`"
							@click="toggleDarkMode()"
						>
							<heroicons-outline-sun
								v-if="!isDarkMode"
								class="fill-current text-gray-100 hover:text-white h-6 w-6"
								aria-hidden="true"
							/>
					...
					<div class="relative z-0 flex-1 px-2 flex lg:gap-2 items-center justify-center md:justify-end">
						<button
							class="hidden md:block rounded-md p-2 hover:bg-musicca-crimson-630 focus:outline-none focus:ring-1 focus:ring-inset focus:ring-white"
							:aria-label="`Switch to ${isDarkMode ? 'light theme' : 'dark theme'}`"
							@click="toggleDarkMode()"
						>
							<heroicons-outline-sun
						...
						<div v-if="route.path !== '/'" ref="searchElement" class="w-full sm:max-w-lg lg:max-w-xs">
							...
							<div class="relative">
								<div class="pointer-events-none absolute inset-y-0 left-0 pl-3 flex items-center" aria-hidden="true">
									<heroicons-outline-search class="h-5 w-5 text-gray-100" />
								</div>
								<input
									id="search"
									v-model="searchInput"
									name="search"
									class="block w-full bg-musicca-crimson-600 border border-transparent rounded-md py-2 pl-10 pr-3 text-base text-white placeholder-gray-200 focus:outline-none focus:bg-musicca-crimson-630 focus:text-gray-100 focus:placeholder-gray-200 focus:ring-2 focus:ring-inset focus:ring-white lg:focus:ring-1"
									placeholder="Search"
									type="search"
									autocomplete="off"
									autocapitalize="off"
								...
								<div
									...
									@click="handleSearch"
								>
									<heroicons-outline-arrow-right class="h-5 w-5 text-gray-100" />
								</div>
								<div
									v-if="isSearchOpen && searchInput && searchResults.length"
									class="absolute mt-1 w-full break-words-legacy border bg-musicca-crimson-600 rounded-md"
								...
								>
									<ul>
										<li
											...
											class="even:bg-musicca-crimson-560 dark:even:bg-musicca-crimson-630 hover:bg-musicca-crimson-630 dark:hover:bg-musicca-crimson-660 rounded-md text-gray-100"
											:class="{
												'ring-1 ring-gray-200 even:bg-musicca-crimson-630 dark:even:bg-musicca-crimson-660 bg-musicca-crimson-630 dark:bg-musicca-crimson-660':
													index === searchScrollPosition,
											}"
										>
											<router-link
												class="block focus:outline-none py-3 px-4 focus-visible:ring-1 focus-visible:ring-gray-200 focus-visible:rounded-md focus-visible:bg-musicca-crimson-630 dark:focus-visible:bg-musicca-crimson-660"
												exact
					...
					<div class="relative z-10 flex items-center md:hidden">
						<button
							type="button"
							class="rounded-md p-2 inline-flex items-center justify-center text-gray-100 hover:bg-musicca-crimson-630 hover:text-white focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white"
			...
			>
				<nav v-if="isOpen" id="mobile-menu" class="md:hidden" aria-label="Global navigation">
					<div class="pt-2 pb-3 px-2 space-y-1">
						<router-link
							to="/"
							class="text-gray-100 hover:bg-musicca-crimson-630 hover:text-white block rounded-md py-2 px-3 text-base font-semibold"
							@click="isOpen = !isOpen"
							>Musicca</router-link
						>
